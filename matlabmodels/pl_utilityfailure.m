...
for k = 1:1000*N^2
    ...
    %p_join = population.*1/2 - steepness.*1/2;
    p_join = population - steepness;

    [s,e] = update(s,e,p_join,p_outtage,p_recover);
    
    if mod(k,N^2)==1
        plotstate(s,e,p_join,lands);
    end
...
function plotstate(s,e,p_join,lands)
...
daspect([1 1 1]);
caxis([-1 1]);
title('Utility state, population')
% colormap(flip(rwb))
ylim([1 Ny]);
xlim([1 Nx]);
subplot(1,3,3);
