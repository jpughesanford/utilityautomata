...
% coeff = [(1 + p1); (-4 - 5*p1); (6 + 10*p1); (-4 - 10*p1); (3*p1 - p2 - p1*p2); p1*p2;];
coeff = [
    (16*p1^4 + 16*p1^5);
...
for k = 1:1000*N^2
    ...
    switch m(i,j)
        case 1
            if m(mod1(i+1,N),j)<0 || m(mod1(i-1,N),j)<0 || m(i,mod1(j+1,N))<0 || m(i,mod1(j-1,N))<0 || rand <= p2
%             v3 = sum(m(:)==-1)/numel(m);
%             if rand<=(1-(1-v3)^4) || rand <= p2
                m(i,j) = -1;
            end
        case 0
            if rand <= p1
